...
(defvar srs-question-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-x q") 'srs-quit-review)
    (define-key map (kbd "C-x r") 'srs-reveal-answer)
    (define-key map (kbd "C-x b") 'srs-review-back)
    (define-key map (kbd "C-x s") 'srs-next-card)
    map)
  "Keymap for `srs-question-mode'.")

(defvar srs-review-answer-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-x e") 'srs-rate-easy)
    (define-key map (kbd "C-x g") 'srs-rate-good)
    (define-key map (kbd "C-x h") 'srs-rate-hard)
    (define-key map (kbd "C-x f") 'srs-rate-forgot)
    (define-key map (kbd "C-x q") 'srs-quit-review)
    (define-key map (kbd "C-x v") 'srs-visit-card-source)
    (define-key map (kbd "C-x b") 'srs-review-back)
    (define-key map (kbd "C-x s") 'srs-next-card)
    map)
...
(defvar srs-cram-answer-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-x n") 'srs-next-card)
    (define-key map (kbd "C-x q") 'srs-quit-review)
    (define-key map (kbd "C-x v") 'srs-visit-card-source)
    (define-key map (kbd "C-x b") 'srs-review-back)
    map)
